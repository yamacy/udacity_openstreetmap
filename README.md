...
For the addr:city k values, there are more problems than one. Some values are not city names, instead they are county names like Esenler. Some are street names, some are city names. So I decided to change values into ones that last word is city name. For example, if a value ends with Esenler but not İstanbul, it is good to add İstanbul after it - Esenler İstanbul.
